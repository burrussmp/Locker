...
const createContentPost = async (req, res) => {
  ...
  S3_Services.uploadSingleMediaS3(
    req,
    res,
    media_meta,
    async (req, res, image) => {
      ...
      try {
        ...
        return S3_Services.deleteMediaS3(req.file.key)
          .then(() => {
            return res
              .status(400)
          ...
          .catch((err2) => {
            return res
              ...
              .json({
                error:
                  "Posting Server Error: Unable to create content for post and failed to clean s3 " +
                  " because " +
                  err.message +
                  " and " +
                  err2.message,
      ...
      try {
        let postData = {
          ...
          tags:
            "tags" in req.body
              ? req.body.tags.split(",").filter((s) => Boolean(s.trim()))
              : [],
        };
        ...
        return S3_Services.deleteMediaS3(req.file.key)
          ...
          .catch((err2) => {
            return res
              ...
              .json({
                error:
                  "Posting Server Error: Unable to clean post, cleaned created content, but failed to clean s3" +
                  " because " +
                  err.message +
                  " and " +
                  err2.message,
...
const getContentPost = async (req, res) => {
  try {
    let post = await Post.findById(req.params.postId)
      .select("type caption tags postedBy createdAt updatedAt")
      .populate({
        path: "content",
...
const editContentPost = async (req, res) => {
  try {
    ...
    "tags" in req.body
      ...
      : undefined;
    let post = await Post.findByIdAndUpdate(req.params.postId, update, {
      runValidators: true,
      new: true,
    });
