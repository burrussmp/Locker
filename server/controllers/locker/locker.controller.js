...
/**
 * @desc Middleware to parse url parameter :lockerProductId
 * @param {Request} req HTTP request object
 * @param {Response} res HTTP response object
 * @param {Function} next Next express middleware function
 * @param {Number} id The ID of the locker product
 * @return {Promise<Response>}
 */
const lockerProductById = async (req, res, next, id) => {
    try {
        const lockerProduct = await LockerProduct.findById(id);
        if (!lockerProduct) {
            return res.status(404).json({ error: StaticStrings.LockerProductControllerErrors.NotFoundError });
        }
        req.locker_product = lockerProduct;
        req.owner = lockerProduct.user;
        return next();
    } catch (err) {
        return res.status(404).json({ error: StaticStrings.LockerProductControllerErrors.NotFoundError });
    }
...
const addProduct = async (req, res) => {
    ...
    try {
        ...
        const newLockerProduct = await (new LockerProduct({user: req.auth._id, locker: req.locker._id, product: req.body.product})).save()
...
const removeProduct = async (req, res) => {
    try {
        return res.status(200).json(await req.locker_product.deleteOne())
    } catch (err) {
        return res.status(400).json({ error: ErrorHandler.getErrorMessage(err) || err.message });
    }
};
