...
const collectionBasicsTest = () => {
  describe('Collection Basics Test', () => {
    ...
    describe(`POST ${url}`, () => {
      ...
      it('Create Collection: Success', async () => agent.post(`${url}?access_token=${admin.access_token}`)
        .attach('hero', newCollectionData.hero)
        .field(getCollectionConstructor(newCollectionData))
        .then((res) => {
          res.status.should.eql(200);
        }));
      it('Create Collection: Not logged in (should fail)', async () => agent.post(`${url}`)
        .attach('hero', newCollectionData.hero)
        .field(getCollectionConstructor(newCollectionData))
        .then((res) => {
      ...
      it('Create Collection: Bad permissions (should fail)', async () => {
        ...
        return agent.post(`${url}?access_token=${admin.access_token}`)
          ...
          .then(async (res) => {
            ...
            const newRole = await RBAC.findOne({ role: 'admin' });
            await Employee.findByIdAndUpdate(admin.id, { permissions: newRole._id });
          });
      });
      ...
      it('Create Collection: Missing hero field which is not required (should succeed)', async () => agent.post(`${url}?access_token=${admin.access_token}`)
        .field(getCollectionConstructor(newCollectionData))
        .then((res) => {
          res.status.should.eql(200);
        }));
      ...
      it('Create Collection: Employee can add a collection (should succeed)', async () => agent.post(`${url}?access_token=${employee.access_token}`)
        .attach('hero', newCollectionData.hero)
        .field(getCollectionConstructor(newCollectionData))
        .then(async (res) => {
          res.status.should.eql(200);
    ...
    describe(`GET ${url}`, () => {
      ...
      it('List Collections: Success', async () => agent.get(`${url}?access_token=${admin.access_token}`).then(async (res) => {
        res.status.should.eql(200);
        res.body.length.should.eql(1);
        res.body[0]._id.should.equal(collection._id);
      }));
      it('List Collections: Not logged in (should succeed)', async () => agent.get(`${url}`).then(async (res) => {
        res.status.should.eql(200);
        res.body.length.should.eql(1);
      }));
      it('List Collections: Logged in (should succeed)', async () => agent.get(`${url}?access_token=${admin.access_token}`).then(async (res) => {
        res.status.should.eql(200);
        res.body.length.should.eql(1);
      }));
