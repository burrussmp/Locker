...
const productSpecificTest = () => {
  describe('Specific Test', ()=>{
    describe('GET /api/ent/products/:productId`', ()=>{
      ...
      it('Get Product: Not logged in (should succeed)', async ()=>{
        return agent.get(`/api/ent/products/${product._id}`).then(async (res) => {
      ...
      it('Get Product: No permissions (should succeed)', async ()=>{
        ...
        return agent.get(`/api/ent/products/${product._id}?access_token=${admin.access_token}`).then(async (res) => {
          res.status.should.eql(200);
          res.body._id.should.eql(product._id);
        });
      });
      ...
      it('Get Product: Product not found (should fail)', async ()=>{
        return agent.get(`/api/ent/products/${anyOrg._id}?access_token=${admin.access_token}`).then(async (res) => {
          res.status.should.eql(404);
          res.body.error.should.eql(StaticStrings.ProductControllerErrors.NotFoundError);
        });
      });
    });
    describe('DELETE /api/ent/products/:productId`', ()=>{
      const agent = chai.request.agent(app);
      let admin; let anyOrg; let product;
      beforeEach(async ()=>{
        await dropDatabase();
      ...
      it('Delete Product: Not logged in (should fail)', async ()=>{
        return agent.delete(`/api/ent/products/${product._id}`).then(async (res) => {
      ...
      it('Delete Product: No permissions (should fail)', async ()=>{
        const role = await RBAC.findOne({'role': 'none'});
        await Employee.findByIdAndUpdate(admin.id, {permissions: role._id});
        return agent.delete(`/api/ent/products/${product._id}?access_token=${admin.access_token}`).then(async (res) => {
          res.status.should.eql(403);
          res.body.error.should.eql(StaticStrings.InsufficientPermissionsError);
        });
      });
      it('Delete Product: Product not found (should fail)', async ()=>{
        return agent.delete(`/api/ent/products/${anyOrg._id}?access_token=${admin.access_token}`).then(async (res) => {
          res.status.should.eql(404);
          res.body.error.should.eql(StaticStrings.ProductControllerErrors.NotFoundError);
        });
      });
      it('Delete Product: Product and requester (not admin) are not part of same organization (should fail)', async ()=>{
        ...
        return agent.delete(`/api/ent/products/${product._id}?access_token=${employeeOtherOrg.access_token}`).then(async (res) => {
          res.status.should.eql(401);
      ...
      it('Delete Product: Supervisor deleting (should succeed)', async ()=>{
        ...
        return agent.delete(`/api/ent/products/${product._id}?access_token=${supervisor.access_token}`).then(async (res) => {
          res.status.should.eql(200);
        });
      });
      it('Delete Product: Employee deleting (should succeed)', async ()=>{
        const employee = await createEmployee(admin, EmployeeData[1]);
        return agent.delete(`/api/ent/products/${product._id}?access_token=${employee.access_token}`).then(async (res) => {
          res.status.should.eql(200);
        });
      });
    ...
    describe('PUT /api/ent/products/:productId`', ()=>{
      const agent = chai.request.agent(app);
      let admin; let anyOrg; let product;
      ...
      it('Update Product: Not logged in (should fail)', async ()=>{
        return agent.put(`/api/ent/products/${product._id}`).send(defaultUpdate).then(async (res) => {
          res.status.should.eql(401);
          res.body.error.should.eql(StaticStrings.UnauthorizedMissingTokenError);
        });
      });
      it('Update Product: No permissions (should fail)', async ()=>{
        const role = await RBAC.findOne({'role': 'none'});
        await Employee.findByIdAndUpdate(admin.id, {permissions: role._id});
        return agent.put(`/api/ent/products/${product._id}?access_token=${admin.access_token}`).send(defaultUpdate).then(async (res) => {
          res.status.should.eql(403);
          res.body.error.should.eql(StaticStrings.InsufficientPermissionsError);
        });
      });
      it('Update Product: Product not found (should fail)', async ()=>{
        return agent.put(`/api/ent/products/${anyOrg._id}?access_token=${admin.access_token}`).send(defaultUpdate).then(async (res) => {
          res.status.should.eql(404);
          res.body.error.should.eql(StaticStrings.ProductControllerErrors.NotFoundError);
        });
      });
      it('Update Product: Product and requester (not admin) are not part of same organization (should fail)', async ()=>{
        await createOrg(admin.access_token, OrganizationData[0]);
        const employeeOtherOrg = await createEmployee(admin, EmployeeData[4]);
        return agent.put(`/api/ent/products/${product._id}?access_token=${employeeOtherOrg.access_token}`).send(defaultUpdate).then(async (res) => {
          res.status.should.eql(401);
          res.body.error.should.eql(StaticStrings.EmployeeControllerErrors.RequireAdminOrRequesterInOrg);
        });
      });
      ...
      it('Update Product: Make request as Supervisor (should succeed)', async ()=>{
        const supervisor = await createEmployee(admin, EmployeeData[0]);
        return agent.put(`/api/ent/products/${product._id}?access_token=${supervisor.access_token}`).send(defaultUpdate).then(async (res) => {
          res.status.should.eql(200);
        });
      });
      it('Update Product: Make request as Employee (should succeed)', async ()=>{
        const employee = await createEmployee(admin, EmployeeData[1]);
        return agent.put(`/api/ent/products/${product._id}?access_token=${employee.access_token}`).send(defaultUpdate).then(async (res) => {
          res.status.should.eql(200);
        });
      });
      it('Update Product: Name update is empty (should fail)', async ()=>{
        const update = {'name': '  '};
        return agent.put(`/api/ent/products/${product._id}?access_token=${admin.access_token}`).send(update).then(async (res) => {
