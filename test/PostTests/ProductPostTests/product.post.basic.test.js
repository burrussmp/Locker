  ...
  describe('Product Post Test Basics', ()=>{
    ...
    describe('GET \'/api/posts\'', ()=>{
      ...
      beforeEach(async ()=>{
        ...
        await agent.post(`/api/posts?access_token=${admin.access_token}&type=ProductPost`).send(reqBody).then((res)=>res.body);
      });
      ...
      it('List Posts: Correct query parameter \'type=ProductPost\' (should succeed and return 1)', async ()=>{
        return agent.get(`/api/posts?access_token=${admin.access_token}&type=ProductPost`).then(async (res)=>{
          res.status.should.eql(200);
          res.body.length.should.eql(1);
        });
      });
